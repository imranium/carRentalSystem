    ...
    <table class="table table-bordered">
        ...
        @foreach($bookings as $booking)
            <tr>
                ...
                <td>
                    <a href="{{ route('bookings.show', $booking) }}" class="btn btn-sm btn-primary mb-1">View</a>

                    @if($booking->status === 'pending')
                        <form action="{{ route('bookings.approve', $booking) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PATCH')
                            <button class="btn btn-sm btn-success mb-1" onclick="return confirm('Approve this booking?')">Approve</button>
                        </form>

                        <form action="{{ route('bookings.reject', $booking) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PATCH')
                            <button class="btn btn-sm btn-danger" onclick="return confirm('Reject this booking?')">Reject</button>
                        </form>
                    @endif
                </td>
            </tr>
